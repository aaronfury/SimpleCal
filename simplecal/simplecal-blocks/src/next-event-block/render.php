...
?>
	<h2 style="simplecal_block_title"><?php echo $attributes['blockTitle']; ?></h2>
	<hr />
<?php
	}
...
?>
	<h3><?php echo $attributes['linkEventTitle'] ? '<a href="' . get_the_permalink() . '">' : null; ?><?php the_title(); ?><?php echo $attributes['linkEventTitle'] ? '</a>' : null; ?></h3>
<?php
		}
?>
	<div class="simplecal_next_event_meta">
		<?php echo $icon_opening_tag; ?>
			<span class="material-symbols-outlined">event</span>
		</div>
		<?php echo $label_opening_tag; ?>
			Date:
		</div>
		<div class="simplecal_next_event_data">
			<?php echo Helper::event_get_the_date("date","start"); ?>
		</div>
	</div>
	<div class="simplecal_next_event_meta">
		<?php echo $icon_opening_tag; ?>
		...
		<div class="simplecal_next_event_data">
			<?php echo Helper::event_get_the_date("time","start"); ?>
		</div>
	</div>
<?php
...
?>
	<div class="simplecal_next_event_meta">
		...
		<div class="simplecal_next_event_meta_data">
			<span class="simplecal_next_event_venue_name"><?php echo $post->simplecal_event_venue_name; ?></span><?php if ($post->simplecal_event_venue_name && ($post->simplecal_event_city || $post->simplecal_event_state)) {?><span class="simplecal_next_event_venue_separator">, </span><?php } ?><span class="simplecal_next_event_city"><?php echo $post->simplecal_event_city; ?></span><?php if ($post->simplecal_event_city && $post->simplecal_event_state) {?><span class="simplecal_next_event_city_separator">, </span><?php } ?><span class="simplecal_next_event_state"><?php echo $post->simplecal_event_state; ?></span>
		</div>
	</div>
<?php
				}
...
<?php
	...
	?>
	<a class="simplecal_all_events_link" href="<?php echo get_post_type_archive_link('simplecal_event'); ?>">View All Events</a>
<?php
	}
}
?>
