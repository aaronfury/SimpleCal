...
const {
  state
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)('nextEventBlock', {
  state: {
    event: ''
  },
  actions: {
    getNextEvent: () => {
      ...
      const queryArgs = [`tags=${context.tags}`];
      apiFetch({
        ...
        path: `/simplecal/v1/events/next-event/?${queryArgs.join('&')}`
      }).then(response => {
